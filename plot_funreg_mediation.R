...
plot.funreg_mediation <- function(fitted_funmed_model,
  ...
  if (what=="coefs") {
    ...
    plot(fitted_funmed_model$original_results$time_grid,
         fitted_funmed_model$original_results$alpha_int_estimate,
         xlab=expression(t),
         ylab=expression(alpha[int](t)),
         main="Intercept for Mediator");
    plot(fitted_funmed_model$original_results$time_grid,
         fitted_funmed_model$original_results$alpha_X_estimate,
         xlab=expression(t),
         ylab=expression(alpha[X](t)),
         main="Treatment on Mediator");
    plot(fitted_funmed_model$original_results$time_grid,
         fitted_funmed_model$original_results$beta_M_estimate,
         xlab=expression(t),
    ...
    est1 <- round(fitted_funmed_model$original_results$beta_int_estimate,2); 
    text(x=1.2,y=3,pos=4,est1);
    text(x=1.1,y=2,pos=4,labels=paste("Indirect effect:"));
    est2 <- round(fitted_funmed_model$bootstrap_results$indirect_effect_boot_estimate,2); 
