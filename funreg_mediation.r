...
#' @param binary_mediator Whether the mediator should be modeled as dichotomous with a logistic model (TRUE), 
#' or numerical with a normal model (FALSE).  
#' @param binary_outcome Whether the outcome should be modeled as dichotomous with a logistic model (TRUE), 
#' or numerical with a normal model (FALSE).  
...
#' \item{alpha_int_estimate}{Estimated intercept function (as a vector of estimates) from the TVEM regression of the mediator on the treatment}
#' \item{alpha_int_se}{Estimated pointwise standard errors associated with the above}
#' \item{alpha_X_estimate}{Estimated time-varying treatment effect from the TVEM regression of the mediator on the treatment}
#' \item{alpha_X_se}{Estimated pointwise standard errors associated with the above}
#' \item{beta_int_estimate}{Estimated scalar intercept from the scalar-on-function regression of the outcome on the mediator and treatment}
#' \item{beta_int_se}{Estimated standard error for the above}
#' \item{beta_X_estimate}{Estimated scalar coefficient for the treatment, from the scalar-on-function regression of the outcome on the mediator and treatment}
#' \item{beta_X_se}{Estimated standard error for the above}
#' \item{beta_M_estimate}{Estimated functional coefficient for the mediator,  from the scalar-on-function regression of the outcome on the mediator and treatment}
#' \item{beta_M_se}{Estimated pointwise standard errors associated with the above}
#' \item{beta_M_pvalue}{The p-value for significance of mediator in predicting outcome, after adjusting for treatment}
#' \item{tau_int_estimate}{Intercept from simple model predicting outcome directly from treatment}
#' \item{tau_int_se}{Estimated standard error for the above}
#' \item{tau_X_estimate}{Coefficient for treatment in model predicting outcome directly from treatment}
#' \item{tau_X_se}{Estimated standard error for the above}
#' \item{indirect_effect_estimate}{Estimated indirect effect, calculated as the dot product of the effect of treatment on mediator and the treatment-adjusted effect of mediator on outcome.  It is a scalar, even though they are functions of time.}
...
#' \describe{
#' \item{indirect_effect_boot_estimate}{Bootstrap point estimate of the indirect effect (average of bootstrap sample estimates).} 
#' \item{indirect_effect_boot_se}{Bootstrap standard error for the indirect effect (standard deviation of bootstrap sample estimates).}
#' \item{indirect_effect_boot_norm_lower}{Lower end of the bootstrap confidence interval using the normal method in boot.ci inthe boot package.}
#' \item{indirect_effect_boot_norm_upper}{Upper end of the bootstrap confidence interval using the normal method.}
#' \item{indirect_effect_boot_basic_lower}{Lower end of the bootstrap confidence interval using the basic method in boot.ci in the boot package.}
#' \item{indirect_effect_boot_basic_upper}{Upper end of the bootstrap confidence interval using the basic method.}
#' \item{indirect_effect_boot_perc_lower}{Lower end of the bootstrap confidence interval using the percentile method in boot.ci inthe boot package.}
#' \item{indirect_effect_boot_perc_upper}{Upper end of the bootstrap confidence interval using the percentile method.}
...
funreg_mediation <- function(data,
                             ...
                             try_log_link=FALSE, # just trying this, not sure if it will work;
                             nboot=199,
                             boot_level=.05) {    
  #-------------------------------------------;
  #--- PROCESSING OF INPUT -------------------;
  ...
  after_boot <- Sys.time(); 
  bootstrap_results <- list(indirect_effect_boot_estimate= norm.ci(boot1,conf=.001)[2],
