...
Notice that the tilde (~) character needs to be included for the covariates.  This is done so that R will allow multiple covariates to be listed in the style of the right side of a formula, e.g., ``~S1 + S2``.   Also, to include the covariate with an assumption of a non-time-varying effect on $M$, you can use code like:
...
By specifying ``tie_covariates_on_mediator`` instead of ``tve_covariates_on_mediator,`` the covariate is assumed to have time-invariant effects rather than time-varying effects. 
