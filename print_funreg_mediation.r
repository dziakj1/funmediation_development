...
print.funreg_mediation <- function(fitted_funmed_model) {
  ...
  cat(fitted_funmed_model$bootstrap_results$indirect_effect_boot_estimate);
  cat("\nIndirect effect bootstrap confidence interval:");
  cat("\n... by normal method:\n");
  cat(c(round(fitted_funmed_model$bootstrap_results$indirect_effect_boot_norm_lower,4), ", ",
        round(fitted_funmed_model$bootstrap_results$indirect_effect_boot_norm_upper,4)));
  cat("\n... by percentile method:\n");
  cat(c(round(fitted_funmed_model$bootstrap_results$indirect_effect_boot_perc_lower,4), ", ",
          round(fitted_funmed_model$bootstrap_results$indirect_effect_boot_perc_upper,4))); 
  cat("\nComputation time:\n");
